...
export const useGroupSettings = (groupid: string) => {
   ...
   const { data } = useQuery({
      // query key to identify the query and queryfn to execute the query
      queryKey: ["group-info"],
      queryFn: () => onGetGroupInfo(groupid),
   })
   // returning the data
   // jsonDescription is a stringified JSON object therefore we need to parse it
   ...
   // setting the icon and thumbnail preview
   useEffect(() => {
   ...
   // we use mutation because we are updating the group settings
...
// this function is used to get the group list based on the query
...
export const useGroupAbout = (
   ...
   // here we need to parse the JSON object
   const jsonContent =
      jsonDescription !== null
         ? JSON.parse(jsonDescription as string)
         : undefined
   // here we are setting the JSON object
   const [onJsonDescription, setJsonDescription] = useState<
      JSONContent | undefined
   >(jsonContent)
   ...
   // function to set the descriptions
   const onSetDescriptions = () => {
      const JsonContent = JSON.stringify(onJsonDescription)
      setValue("jsondescription", JsonContent)
      setValue("description", onDescription)
   ...
   // we use useEffect to set the descriptions if the description changes
   useEffect(() => {
      onSetDescriptions()
      return () => {
         onSetDescriptions()
   ...
   // function that is used to check if user wants to edit the description
...
// function to get the group gallery and update it
// it works by updating the group gallery with the video url or image
