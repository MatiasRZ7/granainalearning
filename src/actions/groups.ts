...
// Function to get affiliate information by ID
export const onGetAffiliateInfo = async (id: string) => {
   try {
      // Find a unique affiliate in the database by their ID
      const affiliateInfo = await client.affiliate.findUnique({
         where: {
            id, // Search condition: affiliate ID
         },
         select: {
            Group: {
               // Select the group associated with the affiliate
               select: {
                  User: {
                     // Select the user associated with the group
                     select: {
                        firstname: true, // Select the user's first name
                        lastname: true, // Select the user's last name
                        image: true, // Select the user's image
                        id: true, // Select the user's ID
                        stripeId: true, // Select the user's Stripe ID
                     },
                  },
               },
            },
      ...
      // If affiliate information is found, return status 200 with the user information
      if (affiliateInfo) {
         return { status: 200, user: affiliateInfo }
      }

      // If no affiliate information is found, return status 404
      return { status: 404 }
   } catch (error) {
      // If an error occurs during the search, return status 400
      return { status: 400 }
   }
}
...
export const onSearchGroups = async (
   ...
   try {
      ...
      // code in the future will be added here to handle post search
      if (mode === "POSTS") {
      }
   } catch (error) {
      return { status: "400", message: "Oops! something went wrong" }
...
// Function to update group settings based on the type of setting
// This works by updating the group settings in the database
...
export const onAddCustomDomain = async (groupid: string, domain: string) => {
   try {
      ...
      //we make a http request to vercel
